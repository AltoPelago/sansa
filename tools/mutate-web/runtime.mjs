...
function enforceTargetSurfaceForWorkbench(plan, targetFormat) {
  const target = normalizeTargetFormat(targetFormat);
  const errors = [];
  for (let operationIndex = 0; operationIndex < plan.operations.length; operationIndex += 1) {
    const operation = plan.operations[operationIndex];
    const result = target === 'json'
      ? validateJsonTargetOperation(operation, operationIndex)
      : validateAeonTargetOperation(operation, operationIndex);
...
function normalizeTargetFormat(targetFormat) {
  return targetFormat === 'json' || targetFormat === 'json-compatible' ? 'json' : 'aeon';
}

function validateAeonTargetOperation(operation, operationIndex) {
  const datatype = operation.datatype;
  if (datatype !== undefined) {
    const datatypeResult = validateAeonTargetDatatype(datatype, operationIndex);
    if (!datatypeResult.ok) return datatypeResult;
  }
  if (!operationHasValue(operation)) return { ok: true };
  const valueResult = validateAeonWorkbenchValue(operation.value, operation, `operations[${operationIndex}].value`);
  if (valueResult.ok) return valueResult;
  return {
    ok: false,
    error: targetSurfaceError(
      'SANSA_MUTATE_TARGET_UNSUPPORTED_VALUE',
      valueResult.message ?? 'Target aeon cannot represent the mutation value',
...
function validateJsonTargetOperation(operation, operationIndex) {
  const addressResult = validateJsonTargetAddresses(operation, operationIndex);
  if (!addressResult.ok) return addressResult;
  const containerResult = validateJsonTargetContainers(operation, operationIndex);
  if (!containerResult.ok) return containerResult;
  const datatype = operation.datatype;
  if (datatype !== undefined) {
    const datatypeResult = validateJsonTargetDatatype(datatype, operationIndex);
    if (!datatypeResult.ok) return datatypeResult;
  }
  const kind = operation.kind;
  if (kind !== undefined) {
    const kindResult = validateJsonTargetDatatype(kind, operationIndex, 'kind');
    if (!kindResult.ok) return kindResult;
  }
  if (!operationHasValue(operation)) return { ok: true };
  return validateJsonTargetValue(operation.value, operationIndex, 'value');
}
...
function targetSurfaceError(code, message, details = {}) {
  return {
    code,
    message,
    phase: 'target',
