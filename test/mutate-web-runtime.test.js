...
test('mutate web example catalog is grouped and uniquely keyed', () => {
  ...
  assert.equal(byId.get('target-aeon-known-kind-mismatch-fail')?.variants.structured.options.targetFormat, 'aeon');
  assert.equal(byId.get('create-object-quoted-field')?.group, 'Typed Values');
  assert.equal(byId.get('create-kind')?.group, 'Attributes and Kinds');
  assert.equal(byId.get('target-aeon-known-kind-mismatch-fail')?.group, 'Target Surfaces');
...
testAeonRuntime('mutate web runtime rejects unsupported policy rewrite fields', async () => {
  const source = readFileSync(new URL('../fixtures/query-inventory.aeon', import.meta.url), 'utf8');
  const policy = {
    default: 'deny',
    rules: [
...
testAeonRuntime('mutate web runtime rejects claimed provenance as policy authority', async () => {
  const source = readFileSync(new URL('../fixtures/query-inventory.aeon', import.meta.url), 'utf8');
  const policy = {
    default: 'deny',
    rules: [
...
testAeonRuntime('mutate web runtime rejects unsupported top-level policy fields', async () => {
  const source = readFileSync(new URL('../fixtures/query-inventory.aeon', import.meta.url), 'utf8');
  const policy = {
    default: 'deny',
    actor: 'Bob',
